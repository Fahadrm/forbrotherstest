...
class CustomerBarcodeController(http.Controller):

    @http.route('/customer_mobile_scan/scan_from_main_menu', type='json', auth='user')
    def main_menu(self, barcode, **kw):
        ...
        """
        ret_open_customer = self.try_open_customer(barcode)
        if ret_open_customer:
            return ret_open_customer
    ...
    def try_open_customer(self, barcode):
        """ If barcode represents a picking, open it
        """
        corresponding_customer = request.env['res.partner'].search([
            ('sequence', '=', barcode),
        ], limit=1)
        if corresponding_customer:
            return self.get_action(corresponding_customer.id)
        return False
    ...
    def get_action(self, picking_id):
        ...
        """
        view_id = request.env.ref('base.view_partner_form').id
        return {
                'action': {
                    'name': _('Open partner form'),
